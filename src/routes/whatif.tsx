    ...
    <AppShell>
      ...
      <div className="px-6 pb-6 grid grid-cols-[380px_1fr] gap-4">
        ...
        <div className="space-y-3">
          ...
          <Card
            title="Outcomes"
            hint="Live recompute · 0.4s"
            actions={<Chip tone="ai">✦ Scenario B</Chip>}
          >
          ...
          <Card title="12-month revenue projection" hint="Baseline vs scenario">
            <div className="p-4">
              ...
              <div className="mt-3 flex items-center gap-4 text-[12px]">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="w-4 h-0.5 bg-ai" />
                  Scenario B
                </span>
          ...
          <Card title="Scenario stack" hint="Saved scenarios · pin 2 to compare">
            <div className="p-4 grid grid-cols-4 gap-3">
              {[
                ...
                <div
                  key={name as string}
                  className={`border rounded-md p-3 text-[12px] ${active ? "ai-surface" : "border-border bg-surface"}`}
                >
              ...
              {[
                ...
                <div key={l as string} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-1.5 bg-muted rounded overflow-hidden">
                    <div
                      className={`h-full ${t === "pos" ? "bg-success" : "bg-destructive"}`}
                      style={{
                        width: `${Math.min(100, Math.abs(parseInt(String(v).replace(/\D/g, ""))) * 0.8)}%`,
                      }}
                    />
                  </div>
                  <span
                    className={`mono w-16 text-right ${t === "pos" ? "text-success" : "text-destructive"}`}
                  >
                    {v}
                  </span>
                </div>
  ...
  return (
    <div>
      ...
      <div className="relative h-6">
        ...
        <div
          className="absolute top-1/2 -translate-y-1/2 h-1 bg-ai rounded"
          style={{ width: `${pct * 100}%` }}
        />
        <div
          className="absolute top-1/2 -translate-y-1/2 size-3.5 rounded-full bg-surface border-2 border-ai"
...
function ScenarioChart() {
  ...
  const min = 2380,
    max = 2620;
  const toPath = (v: number[]) =>
    v.map((y, i) => `${(i / (v.length - 1)) * w},${h - ((y - min) / (max - min)) * h}`).join(" ");
  return (
