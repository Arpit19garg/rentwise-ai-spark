  ...
  return (
    <AppShell>
      ...
      <div className="h-12 border-b border-border bg-surface flex items-center gap-3 px-5 sticky top-0 z-10">
        ...
        <span className="text-[12px] text-muted-foreground">Atlanta, GA 30309 · 2BR / 2BA · 1,140 sqft</span>
        <div className="ml-auto flex items-center gap-2">
          ...
          <Btn variant="primary">Approve  <span className="mono opacity-70">A</span></Btn>
        </div>
      </div>
      ...
      <div className="grid grid-cols-[320px_1fr_380px] min-h-[calc(100vh-7rem)]">
        ...
        <aside className="border-r border-border bg-surface p-4 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-4 gap-1">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="wf-placeholder aspect-square text-[9px]">{i === 3 ? "+5" : ""}</div>
            ))}
          </div>
          ...
          <div>
            <div className="text-[11px] uppercase text-muted-foreground tracking-wider mb-1.5">Amenities</div>
          ...
          <div>
            <div className="text-[11px] uppercase text-muted-foreground tracking-wider mb-2">Lease history</div>
        ...
        <section className="p-5 overflow-y-auto space-y-4 bg-background">
            ...
            <div className="flex items-start gap-5">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-muted-foreground mono">model v4.1 · 11 comps · updated 6m ago</span>
                </div>
                ...
                <div className="mt-5 flex gap-2">
                  <Btn variant="primary" size="md">Approve</Btn>
                  <Btn variant="secondary" size="md">Adjust</Btn>
                  <Btn variant="ai" size="md">✦ Run what-if</Btn>
                  <Btn variant="ghost" size="md">Explain</Btn>
                </div>
              </div>
          ...
          {/* Drivers */}
          <Card title="Key drivers" hint="SHAP-style contributions to recommended price" actions={<Link to="/explainability" className="text-[12px] text-ai hover:underline">Open explainability ↗</Link>}>
            <div className="p-4 space-y-2">
              {[
                ...
                <DriverBar key={label as string} label={label as string} value={val as number} tone={tone as any} />
              ))}
            </div>
          </Card>

          {/* Tabs */}
          <Card title="Comparable properties" hint="Top 5 of 11 · sorted by AI relevance" actions={<Link to="/comps" className="text-[12px] text-ai hover:underline">View all 11 ↗</Link>}>
          ...
          <Card title="How the AI calculated this price">
            <ol className="p-4 space-y-2 text-[12.5px] list-decimal pl-8 marker:text-muted-foreground">
              <li>Selected <span className="font-medium">11 comparable units</span> within 1.5mi · 2BR / 2BA · leased in last 90 days <sup className="text-ai">[1]</sup></li>
              <li>Applied submarket trend <span className="mono">+1.4% MoM</span> (Midtown Atlanta) <sup className="text-ai">[2]</sup></li>
              <li>Adjusted +$179 for positive features, −$58 for negative features (see drivers)</li>
              <li>Calibrated against last 90 days of leases · segment MAPE <span className="mono">3.1%</span></li>
        ...
        <aside className="border-l border-border bg-surface flex flex-col">
          <div className="h-11 px-4 flex items-center gap-2 border-b border-border">
            <div className="size-5 rounded grid place-items-center bg-ai-soft text-ai text-[10px]">✦</div>
            <div className="text-[13px] font-semibold">AI Copilot</div>
            <Chip tone="ai">Context: Unit 4B</Chip>
            <Btn variant="ghost" className="ml-auto">⋯</Btn>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 text-[12.5px]">
            ...
            <Msg who="ai">
              <p>
                <span className="font-medium">3322 Buford</span> is 1.4 mi away in a different submarket (Brookhaven) with{" "}
                <span className="font-medium">−0.6% rent trend</span><sup className="text-ai">[1]</sup>, while{" "}
                <span className="font-medium">1180 Peachtree</span> is 0.2 mi away in the same Midtown core with matching school
                zone and walkability<sup className="text-ai">[2]</sup>.
              </p>
              <div className="mt-2 border border-border rounded-md p-2 bg-surface">
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground mb-1">Tool · queried_comps</div>
                <div className="mono text-[11px]">11 results · 0.6s</div>
              </div>
            </Msg>
            <Msg who="user">What if we keep rent flat and add covered parking?</Msg>
            <Msg who="ai">
              <p>Adding covered parking (~$45/mo amenity value in this submarket) at flat rent of $2,395 would:</p>
              <ul className="list-disc pl-4 mt-1 space-y-0.5">
                <li>Reduce time-to-lease by <span className="mono">~3.4 days</span></li>
          ...
          <div className="px-4 pb-2 flex flex-wrap gap-1.5">
            {["Why this price?", "What if +5%?", "Hide highway comps", "Draft justification"].map((p) => (
              <button key={p} className="ai-chip text-[11px]">✦ {p}</button>
            ))}
          </div>
    ...
    <div className="flex items-center gap-3 text-[12px]">
      ...
      <div className="flex-1 h-5 grid grid-cols-2 relative">
        <div className="border-r border-border-strong relative">
          {!isPos && (
            <div className="absolute right-0 top-0 h-full bg-destructive/70 rounded-l-sm" style={{ width: `${pct}%` }} />
          )}
        </div>
        <div className="relative">
          {isPos && <div className="absolute left-0 top-0 h-full bg-success/70 rounded-r-sm" style={{ width: `${pct}%` }} />}
        </div>
      </div>
  ...
  if (who === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] bg-accent rounded-lg rounded-br-sm px-3 py-2 text-[12.5px]">{children}</div>
      </div>
    );
  }
  return (
    <div className="flex gap-2">
      <div className="size-6 rounded-md ai-surface grid place-items-center text-ai text-[11px] shrink-0">✦</div>
